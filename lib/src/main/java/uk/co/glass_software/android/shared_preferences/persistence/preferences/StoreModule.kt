...
internal class StoreModule(private val context: Context,
    ...
    @Singleton
    @Named(PLAIN_TEXT)
    fun provideStore(@Named(PLAIN_TEXT) store: SharedPreferenceStore) = store

    @Provides
    @Singleton
    @Named(ENCRYPTED)
    fun provideEncryptedStore(store: EncryptedSharedPreferenceStore) = store

    @Provides
    @Singleton
    @Named(LENIENT)
    fun provideLenientStore(store: LenientEncryptedStore) = store

    @Provides
    @Singleton
    @Named(FORGETFUL)
    fun provideForgetfulStore(store: ForgetfulEncryptedStore) = store
    ...
    companion object {
        internal val MAX_FILE_NAME_LENGTH = 127
